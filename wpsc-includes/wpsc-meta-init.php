 ...
 *
 */
function wpsc_meta_register_type( $meta_object_types = null ) {
...
add_action( 'init' , 'wpsc_meta_register_tables' , 1 );
add_action( 'switch_blog' , 'wpsc_meta_register_tables' );
...
function wpsc_meta_core_object_types() {
	$meta_object_types = array( 'cart_item', 'customer' );
...
function wpsc_check_meta_access_functions( $meta_object_type ) {
	...
	if ( ! file_exists( $meta_functions_file ) ) {	
		$template = file_get_contents( dirname( __FILE__ ) .'\wpsc_meta_functions_template.txt' );
...
function wpsc_meta_functions_file( $meta_object_type ) {
	$meta_functions_file = dirname( __FILE__ ) .'\wpsc_meta_'.$meta_object_type.'.php';
 ...
 * is that plugins or themese that use custom meta types won't have the tables until after the
 * first page is viewed when that page implements the filter that defines the custom meta types.
