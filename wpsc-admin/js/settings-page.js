...
(function($){
	...
	WPSC_Settings_Page.Gateway = {
		event_init : function() {
			...
			wrapper.delegate('.edit-payment-module-cancel', 'click', WPSC_Settings_Page.Gateway.event_edit_payment_gateway_cancel);
		},
		event_edit_payment_gateway : function() {
			var element = $(this), // edit link
				...
				ajax_callback = function(response) {
					...
					$('#wpsc_gateway_settings_'+ payment_gateway_id).show().append(response.obj.content);
					// hide edit link
					element.hide();
				};
		...
		event_edit_payment_gateway_cancel : function() {
			...
			// show edit link
			element.closest('.wpsc-select-gateway').find('.edit-payment-module').show();
			return false;
		}
	};
