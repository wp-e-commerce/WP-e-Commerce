...
function wpsc_transaction_theme() {
		...
		switch( $selected_gateway ){
			...
			case 'paystation':
				...
				if($result == 'Insufficient Funds' && $ec == 5){
					...
					$payment_instructions = printf( __( 'Sorry, your transaction was not accepted due to insufficient funds <br /><a href="%1$s">Click here to go back to the checkout page</a>.', 'wp-e-commerce' ), get_option( "shopping_cart_url" ) );
				}
				if ( $processed_id )
	...
	if( ! $dont_show_transaction_results ) {
		...
		printf( __( 'Sorry, your transaction was not accepted.<br /><a href="%1$s">Click here to go back to the checkout page</a>.', 'wp-e-commerce' ), get_option( "shopping_cart_url" ) );
	}
}
